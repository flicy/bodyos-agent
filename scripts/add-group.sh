...
#
# 例：
#   ./add-group.sh bodyos oc_bebdb4d6... 健康搭子打卡群 buddy
#
...
echo "→ Body OS add-group: profile=$PROFILE chat=$CHAT_ID 群=$GROUP_NAME theme=$THEME"
...
case "$THEME" in
  system|buddy)
    H cron create "30 8 * * *" "你是 Body OS 健身管理专家。现在是「$GROUP_NAME」的早晨打卡。只输出一条将直接发送到群里的简短中文消息，不调用任何工具，不提及体重或私人数据。邀请大家用一行打卡：今天的运动或健康计划。明确可跳过或降级，不追问。不要输出分析、标题说明或[SILENT]。" --name "$GROUP_NAME·早晨打卡" --deliver "$DELIVER" --profile "$PROFILE" >/dev/null
    H cron create "30 20 * * *" "你是 Body OS 健身管理专家。现在是「$GROUP_NAME」的 30 秒晚间复盘。只输出一条将直接发送到群里的简短中文消息，不调用任何工具，不公开敏感数据。请参与者用一行回答：今天完成了什么、最大的阻力、明天愿意保留的一个最小行动。提醒评分看行为完成度，不比体重；漏答不惩罚。不要输出分析、标题说明或[SILENT]。" --name "$GROUP_NAME·晚间复盘" --deliver "$DELIVER" --profile "$PROFILE" >/dev/null
    H cron create "0 21 * * 0" "你是 Body OS 健身管理专家，要为「$GROUP_NAME」生成周日低压力周报。只读取群 $CHAT_ID 最近7天与饮食、睡眠、运动、恢复、打卡和互相支持有关的公开消息；不要读取或泄露任何人的单聊、体重、围度、疾病、用药或其他私密信息。使用 lark-im 只读能力，最多读取最近200条群消息。按“本周做得好的行为、主要阻力、各自下周一个微目标、共同约定”输出一条简洁中文群消息；数据不足时明确说数据不足并发出复盘邀请，不编造。不要使用发送工具，最终输出会由定时任务直接投递。" --name "$GROUP_NAME·周日合作周报" --deliver "$DELIVER" --profile "$PROFILE" --skill lark-im --skill lark-shared >/dev/null
    echo "  已建节奏 cron：早打卡/晚复盘/周报"
    ;;
  wellness)
    H cron create "0 9 * * *" "你是 Body OS 健身管理专家。现在是「$GROUP_NAME」的每日最小健康行动邀请。只输出一条将直接发送到群里的简短中文消息，不调用任何工具，不提及体重或私人数据。这个群主打轻松氛围，弱数据、强鼓励。邀请大家用一行说出今天愿意做的一个最小健康行动，并强调可跳过、不追问。不要输出分析、标题说明或[SILENT]。" --name "$GROUP_NAME·每日最小行动" --deliver "$DELIVER" --profile "$PROFILE" >/dev/null
    H cron create "0 21 * * *" "你是 Body OS 健身管理专家。现在是「$GROUP_NAME」的晚间互赞时间。只输出一条将直接发送到群里的简短中文消息，不调用任何工具，不公开敏感数据。邀请大家用一行分享今天做到的一个健康小事，并互相点赞鼓励；语气温暖积极。不比较数据、不评判、漏答不惩罚。不要输出分析、标题说明或[SILENT]。" --name "$GROUP_NAME·晚间互赞" --deliver "$DELIVER" --profile "$PROFILE" >/dev/null
